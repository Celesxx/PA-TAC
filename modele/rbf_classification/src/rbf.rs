...
impl RBFModel {
    pub fn init(points: &[Point], y_train: &[f64], k: usize, max_iter: usize, gamma: f64) -> Self {
        let normalized_points = normalize_points(points);
        let model = train_rbf_model(&normalized_points, k, max_iter);
        let rbf_matrix = compute_rbf_matrix(&normalized_points, &model, gamma);
    ...
    pub fn predict(&self, x_test: &[Point]) -> Vec<f64> {
        let normalized_test_points = normalize_points(x_test);
        rbf_classification(&normalized_test_points, self, self.gamma, &self.weights)
    }
}
...
pub fn train_rbf_classification(
    x_train: &[Point],
    y_train: &[f64],
    k: usize,
    max_iter: usize,
...
pub fn rbf_classification(
    ...
    let raw_outputs = rbf_matrix.dot(&ArrayView1::from(weights)).to_vec();
    raw_outputs.iter().map(|&x| if x >= 0.5 { 1.0 } else { 0.0 }).collect()
}

pub fn normalize_points(points: &[Point]) -> Vec<Point> {
    let mut min_vals = vec![f64::MAX; points[0].data.len()];
    let mut max_vals = vec![f64::MIN; points[0].data.len()];

    for point in points {
        for (i, &val) in point.data.iter().enumerate() {
            if val < min_vals[i] {
                min_vals[i] = val;
            }
            if val > max_vals[i] {
    ...
    points.iter()
        .map(|point| {
            Point::new(
                point.data.iter()
                    .enumerate()
                    .map(|(i, &val)| (val - min_vals[i]) / (max_vals[i] - min_vals[i]))
                    .collect()
            )
        })
        .collect()
...
pub extern "C" fn train_rbf_model_ffi(
    ...
    let (model, weights) = train_rbf_classification(&x_train, y_train, k as usize, max_iter as usize, gamma);
...
#[no_mangle]
pub extern "C" fn rbf_classification_ffi(
    ...
    let preds = rbf_classification(&x_test, unsafe { &*model }, gamma, weights);
